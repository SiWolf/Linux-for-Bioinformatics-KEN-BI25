...
Use Nano to create a script called for_basename.sh with the following content. Adjust the path of the in_dir variable so it leads to your current working directory. Use a absolute path.
...
_Utilisez Nano pour créer un script appelé for_basename.sh avec le contenu suivant. Ajustez le chemin de la variable in_dir pour qu'il mène à votre répertoire de travail actuel. Utilisez un chemin absolu._
